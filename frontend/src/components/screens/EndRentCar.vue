...
onMounted(async () => {
  try {
    ...
    state.value.rentals_id = car.value.id;
    state.value.plate_number = car.value.plate_number;// Set rentals_id dari data mobil
  } catch (error) {
...
const endRentCar = async () => {
  try {
    console.log(state.value.plate_number)
    ...
    <div class="w-1/2 h-screen bg-white items-center justify-between p-12">
      ...
      <div v-if="car" class="mt-8">
        ...
        <div class="w-full p-2 border-2 border-secondary--300 mt-8 mb-4">
          ...
          <input v-model="car.plate_number" readonly class="w-full font-medium text-base text-primary--500 outline-none mt-2" type="text" />
        </div>
      </div>
