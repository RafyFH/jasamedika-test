...
class RentalsController extends Controller
{
    public function index(Request $request) {
        $query = DB::table('rentals as a')
            ->join('users as u', 'u.id', '=', 'a.user_id')
            ->join('cars as c', 'c.id', '=', 'a.car_id')
            ->select('a.*', 'u.name', 'c.brand', 'c.model', 'c.plate_number', 'u.name');

        // Filter berdasarkan nilai filter
        if ($request->has('filter')) {
            $filter = $request->input('filter');

            if ($filter === 'ongoing') {
                $query->where('a.is_return', '!=' , 'true'); // Ongoing rentals
            } elseif ($filter === 'history') {
                $query->where('a.is_return', 'true'); // Returned rentals
            }
            // Jika filter adalah 'all', kita tidak perlu menambahkan kondisi apapun
        }
    ...
    public function show($id) {
        ...
        ->join('users as u', 'u.id', '=', 'a.user_id')
        ->join('cars as c', 'c.id', '=', 'a.car_id')
        ->select('a.*', 'u.name as user_name', 'c.brand', 'c.model', 'c.plate_number')
        ->where('a.id', '=', $id)
        ->first();
